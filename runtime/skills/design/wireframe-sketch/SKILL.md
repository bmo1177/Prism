...
description: |
  ...
  "wireframe", "sketch wireframe", "hand-drawn", "lo-fi", or "whiteboard".
---
...
1. **Skip finished UI** — this skill explicitly wants a low-fidelity look.
   Use system serif for headlines, mono for annotations, with a marker font
   fallback.
...
3. **Layout**, in order:
   - **Page header** — bold serif title with a "WIREFRAME v0.1" tag
     ...
     the right in mono.
   - **Tab strip** — 4–5 labels with marker check-square glyphs. The
     active one has a highlighter swipe behind it (yellow/orange tint +
     slight skew).
   ...
   - **Bar chart placeholder** — a card with hatched-fill rectangles of
     varying heights.
   - **Sticky notes** — 1–2 yellow/pink notes with marker text, taped with
     a slightly rotated band, pinned over key regions to call out "next
     step", "page-1", or "needs review".
