...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// Craft app version that recorded this.
    pub app: String,
...
/// Browsing query over ALL recorded versions — the read model for the global
/// Provenance page. Pages are keyed by *file order index*, not by timestamp:
/// several versions can share the same clock second, so a ts cursor would skip
/// or duplicate rows; the append-order index pages exactly. Newest first.
#[derive(Default, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ProvenanceQuery {
    /// Only records authored in one session (its `session_id`).
    #[serde(default)]
    pub session_id: Option<String>,
    /// Case-insensitive substring over the path, tool, and log note.
    #[serde(default)]
    pub search: Option<String>,
    /// Keyset cursor: only records strictly OLDER (earlier in append order)
    /// than this index — the `next` of the previous page.
    #[serde(default)]
...
pub struct ProvenancePage {
    pub rows: Vec<ProvenanceRecord>,
    /// Total rows matching the filters (for the header count).
    pub total: usize,
    /// Cursor for the next (older) page: the file-order index of the last row
    /// returned. Absent at the end.
    pub next: Option<usize>,
}

/// Cap on a single page so a bad `limit` can't force a giant clone.
const MAX_PAGE: usize = 1000;
const DEFAULT_PAGE: usize = 500;

/// The query core, separated from the AppHandle so it is unit-testable.
pub fn query_store(root: &Path, query: &ProvenanceQuery) -> Result<ProvenancePage, String> {
    let all = read_all(&store_file(root));
    let limit = query.limit.unwrap_or(DEFAULT_PAGE).min(MAX_PAGE);
    // Newest first = reverse append order; keep each record's file-order index
    // so a cursor page resumes exactly (no clock-second ties to break).
    let mut idx: Vec<usize> = (0..all.len()).rev().collect();
    if let Some(sid) = &query.session_id {
        let sid = sid.as_str();
        idx.retain(|&i| all[i].session_id.as_deref() == Some(sid));
    }
    if let Some(term) = query.search.as_ref() {
        ...
        idx.retain(|&i| {
            ...
            r.path.to_lowercase().contains(&needle)
                || r.tool.to_lowercase().contains(&needle)
                || r.log.as_deref().is_some_and(|l| l.to_lowercase().contains(&needle))
        });
    }
    if let Some(before) = query.before_index {
        idx.retain(|&i| i < before);
    }
    let total = idx.len();
    let rows: Vec<ProvenanceRecord> = idx.iter().take(limit).map(|&i| all[i].clone()).collect();
    let next = if idx.len() > limit { Some(idx[limit - 1]) } else { None };
    Ok(ProvenancePage { rows, total, next })
}

/// `async`: reads the whole (unbounded) store off the UI thread; `limit` bounds
/// the clone the page returns.
#[tauri::command(async)]
pub fn query_provenance(app: AppHandle, query: ProvenanceQuery) -> Result<ProvenancePage, String> {
    query_store(&workspace_dir(&app)?, &query)
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn query_pages_newest_first_and_filters_by_session_and_text() {
        use super::{query_store, ProvenanceQuery};
        let root = temp_root("query");
        // Three records across two sessions, distinct tools and notes.
        append_record(&root, "a.md", "write", Some("s1".into()), Some("m1".into()), Some("hello".into()), None, Some("wrote a".into()), None, None).unwrap();
        append_record(&root, "b.py", "write", Some("s2".into()), None, Some("print('x')".into()), None, Some("wrote b".into()), None, None).unwrap();
        append_record(&root, "c/fig.md", "run", Some("s2".into()), None, None, Some("@@ -1 +1 @@".into()), Some("rendered fig".into()), None, Some("run_9".into())).unwrap();

        // Default: every record, newest first (reverse append order).
        let page = query_store(&root, &ProvenanceQuery::default()).unwrap();
        assert_eq!(page.total, 3);
        assert!(page.next.is_none());
        let paths: Vec<_> = page.rows.iter().map(|r| r.path.as_str()).collect();
        assert_eq!(paths, vec!["c/fig.md", "b.py", "a.md"]);

        // Session filter keeps only that session's authored writes.
        let q = ProvenanceQuery { session_id: Some("s2".into()), ..Default::default() };
        let page = query_store(&root, &q).unwrap();
        assert_eq!(page.total, 2);
        let paths: Vec<_> = page.rows.iter().map(|r| r.path.as_str()).collect();
        assert_eq!(paths, vec!["c/fig.md", "b.py"]);

        // Search matches path, tool, and log note alike (case-insensitive).
        for (term, want) in [("C/FIG.MD", vec!["c/fig.md"]), ("run", vec!["c/fig.md"]), ("wrote b", vec!["b.py"])] {
            let q = ProvenanceQuery { search: Some(term.into()), ..Default::default() };
            let page = query_store(&root, &q).unwrap();
            let paths: Vec<_> = page.rows.iter().map(|r| r.path.as_str()).collect();
            assert_eq!(paths, want, "unexpected match for {term:?}");
        }
    ...
    #[test]
    fn query_keyset_paging_resumes_exactly() {
        use super::{query_store, ProvenanceQuery};
        let root = temp_root("paging");
        for name in ["a.md", "b.py", "c.md"] {
            append_record(&root, name, "write", None, None, Some("x".into()), None, None, None, None).unwrap();
        }
        // Force two records into the same clock second by patching the stored ts:
        // a timestamp cursor would tie, file-order paging must not duplicate or skip.
        let file = root.join(".openscience/provenance.jsonl");
        let text = std::fs::read_to_string(&file).unwrap();
        let mut records: Vec<serde_json::Value> = text.lines().map(|l| serde_json::from_str(l).unwrap()).collect();
        let ts = records[0]["ts"].as_u64().unwrap();
        for r in &mut records {
            r["ts"] = serde_json::json!(ts);
        }
        let out: Vec<String> = records.iter().map(|r| r.to_string()).collect();
        ...
        for _ in 0..3 {
            ...
            seen.push(page.rows[0].path.clone());
            before = page.next;
            if before.is_none() {
                break;
            }
        }
        assert_eq!(seen, vec!["c.md", "b.py", "a.md"]);
        assert!(before.is_none(), "the last page reports next=None exactly once");
