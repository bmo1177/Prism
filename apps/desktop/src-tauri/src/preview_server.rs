...
fn handle<F: Fn(&str) -> Option<PathBuf>>(mut stream: TcpStream, token: &str, root_of: &F) {
    ...
    // the base folder, /d/<path> the bundled design templates — each resolved
    // (and sandboxed) against its own root.
...
/// chooses the tree: the active workspace (default), the base folder, or the
/// bundled design templates (`"design"`).
#[tauri::command]
pub fn preview_url(
    app: AppHandle,
    ...
    let (port, token) = match guard.clone() {
        ...
        None => {
            ...
            let p = serve(&token, move |scope| match scope {
                ...
                "d" => crate::design_templates::bundled_design_dir(&handle),
                _ => None,
            })
    ...
    let root_dir = if root.as_deref() == Some("design") {
        crate::design_templates::bundled_design_dir(&app)
            .ok_or_else(|| "bundled design templates unavailable".to_string())?
    } else {
        crate::artifact_file::scope_root(&app, root.as_deref())?
    };
    let rel = relativize(&root_dir, &path)?;
