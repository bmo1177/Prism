// Craft — Tauri 2 entry. Hosts the React frontend and supervises the
// bundled OpenCode sidecar (isolated config/data + dedicated port; killed on exit).
mod artifact_file;
mod browser;
mod cron;
mod debug_log;
mod design_templates;
mod science_skills;
mod examples;
mod gateway;
mod git_snapshot;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Wake the scheduled-jobs loop; it polls jobs.db every tick.
            cron::start_scheduler(app.handle());
            Ok(())
        })
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            design_templates::list_design_templates,
            science_skills::list_science_skills,
        ...
        .expect("error while building Craft")
        .run(|app, event| {
